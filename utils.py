...
def train_one_epoch(args, model, train_loader, loss_fn, optimizer, epoch=None):
    ...
    # Fetch the first batch to inspect its content
    first_batch = next(iter(train_loader))
    # print(f"First batch type: {type(first_batch)}, First batch content: {first_batch}")
...
def test_classification(args, checkpoint, data_loader_test):
    ...
    with torch.no_grad():
        for _, (samples, targets) in enumerate(tqdm(data_loader_test)):
            ...
            y_test = torch.cat((y_test, targets.cpu()), 0)
            ...
            varInput = torch.autograd.Variable(samples.view(-1, c, h, w).to(args.device))
            ...
            p_test = torch.cat((p_test, outMean.cpu().data), 0)
